...
import { checkProfileUsernameAvailability, cliamUsername } from "@/modules/profile/actions"
...
const ClaimLinkForm = () => {
    ...
    const handleSubmit = async (e: React.SubmitEvent) => {
        try {
            ...
            if (linkValue.trim() && isAvailable) {
                setIsClaiming(true);
                const result = await cliamUsername(linkValue);
                if (result.success) {
    ...
    const displayOrigin = origin ? origin.replace("https://", "").replace("http://", "") : "treebio.com";
