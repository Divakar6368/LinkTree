      ...
      <CardContent >
        <div className="flex items-start gap-3">
          <Button
            variant="ghost"
            size="sm"
            className="h-6 w-6 p-0 cursor-grab opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-1"
          >
            <Grip size={14} />
          </Button>
...
export const LinkFormWithPreview: React.FC<LinkFormWithPreviewProps> = ({
  ...
  const getSafeHostname = (urlString: string): string => {
    try {
      const formattedUrl = urlString.startsWith("http://") || urlString.startsWith("https://")
        ? urlString
        : `https://${urlString}`;
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">
                    {ogData?.siteName || getSafeHostname(url)}
                  </p>
                </div>
              </div>
            </div>
