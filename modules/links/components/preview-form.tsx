...
const PreviewFrame = async ({ links }: PreviewFrameProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full min-h-[500px] sm:min-h-[620px] py-4">
      {/* Dynamic Scaling Wrapper */}
      <div className="transform scale-[0.80] xs:scale-[0.88] sm:scale-100 transition-transform origin-top flex justify-center w-full">
        {/* Phone Frame */}
        <div className="w-[280px] h-[580px] bg-zinc-700 rounded-[2.5rem] p-2 shadow-2xl flex-shrink-0">
          {/* Screen */}
          <div className="w-full h-full bg-gray-50 rounded-[2rem] overflow-hidden relative">
            {/* Status Bar */}
            <div className="absolute top-0 left-0 right-0 h-6 bg-transparent flex items-center justify-center z-10">
              <div className="w-20 h-1 bg-zinc-700 rounded-full"></div>
            </div>

            {/* Content */}
            <div className="pt-8 pb-4 px-4 h-full flex flex-col justify-between">
              <div>
                {/* Profile Section */}
